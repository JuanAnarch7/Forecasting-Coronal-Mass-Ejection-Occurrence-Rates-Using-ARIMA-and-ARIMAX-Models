...
forecast_sarima_obj = resultado_sarima.get_forecast(steps=len(df_test))
forecast_sarima_mean = forecast_sarima_obj.predicted_mean
forecast_sarima_ci = forecast_sarima_obj.conf_int(alpha=0.05)
forecast_sarima_series = pd.Series(forecast_sarima_mean.values, index=endog_test.index)
sarima_ci_lower = forecast_sarima_ci.iloc[:, 0].values
sarima_ci_upper = forecast_sarima_ci.iloc[:, 1].values
...
# Out-of-sample forecast with confidence intervals
forecast_sarimax_obj = resultado_sarimax.get_forecast(steps=len(df_test), exog=exog_test)
forecast_sarimax_mean = forecast_sarimax_obj.predicted_mean
forecast_sarimax_ci = forecast_sarimax_obj.conf_int(alpha=0.05)
forecast_sarimax_series = pd.Series(forecast_sarimax_mean.values, index=endog_test.index)
sarimax_ci_lower = forecast_sarimax_ci.iloc[:, 0].values
sarimax_ci_upper = forecast_sarimax_ci.iloc[:, 1].values
...
# ========== FIGURE 1: FORECAST COMPARISON WITH CONFIDENCE INTERVALS ==========
...
# Plot forecasts (test) with 95% confidence intervals
ax1.plot(test_dates, y_test_pred_sarima,
         ...
         zorder=2, alpha=0.9)
ax1.fill_between(test_dates, sarima_ci_lower, sarima_ci_upper,
                 alpha=0.15, color=COLOR_ARIMA, label='SARIMA 95% CI')
...
ax1.plot(test_dates, y_test_pred_sarimax,
         ...
         zorder=2, alpha=0.9)
ax1.fill_between(test_dates, sarimax_ci_lower, sarimax_ci_upper,
                 alpha=0.15, color=COLOR_ARIMAX, label='SARIMAX 95% CI')
...
# Build CI columns for test period (NaN for training)
n_train = len(train_dates)
n_test = len(test_dates)
sarima_lower_all = np.concatenate([np.full(n_train, np.nan), sarima_ci_lower])
sarima_upper_all = np.concatenate([np.full(n_train, np.nan), sarima_ci_upper])
sarimax_lower_all = np.concatenate([np.full(n_train, np.nan), sarimax_ci_lower])
sarimax_upper_all = np.concatenate([np.full(n_train, np.nan), sarimax_ci_upper])
...
df_results = pd.DataFrame({
    ...
    "SARIMA_CI_lower": sarima_lower_all,
    "SARIMA_CI_upper": sarima_upper_all,
    "SARIMAX_CI_lower": sarimax_lower_all,
    "SARIMAX_CI_upper": sarimax_upper_all,
...
with open('model_metrics_summary_monthly.txt', 'w', encoding='utf-8') as f:
    ...
    f.write(f"  CME filters: speed >= {MIN_SPEED} km/s, {MIN_WIDTH}deg <= width <= {MAX_WIDTH}deg\n")
    ...
    f.write(f"  ADF test (stationarity): stat={adf_result[0]:.4f}, p={adf_result[1]:.4f}, lags={adf_result[2]}\n")
    if adf_result[1] < 0.05:
    ...
    f.write(f"  R2:   {mejora_r2:+.2f}%\n")
    f.write(f"  DAIC: {mejora_aic:+.2f}\n\n")

    f.write("="*80 + "\n")
    f.write("*** Test metrics represent TRUE out-of-sample forecasting performance\n")
    f.write("    95% prediction intervals saved in CSV output columns *_CI_lower/*_CI_upper\n")
    f.write("="*80 + "\n")
...
print("\nKey findings:")
print(f"  - Stationarity: ADF p-value = {adf_result[1]:.4f} (lags = {adf_result[2]})")
